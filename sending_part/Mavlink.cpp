...
const float Mavlink::karelGetLatitude() {
    return latitude;
}

const float Mavlink::karelGetLongitude() {
    return longitude;
}
...
const int Mavlink::getNumberOfSatelites() {
        return numberOfSatelites;
}

const float Mavlink::getGpsGroundSpeed() {
    return gpsGroundSpeed /** 0.0194384f */;  //snad by mely byt metry za vterinu, oni to tady konvertovali do uzlu nebo ceho
}

const float Mavlink::getAltitude() {
    return altitude /* / 100.0f*/;      //kdyz nedelim stem tak mi to vychazi spravne
}
...
bool Mavlink::parseMessage(char c) {
    ...
    if (mavlink_active == 0 && millis() < 20000 && millis() > 5000) {
        if (c == '\n' || c == '\r') {crlf_count++;} else {crlf_count = 0;}
    }
